...
To understand the idea of online mirror descent lets revisit projected online gradient descent and formulate it in a more abstract way. Given an input point $x \gets x_t$ and an impulsive vector $y \gets \gamma_t v_t$, then \ref{equ:POGD} returns an output point $x^+ \gets x_{t+1}$ defined as
...
Now, the regret of an algorithm measures how "sorry" the learner is, to not have followed a fixed competing strategy in hindsight. It is the difference between the cumulative loss of the sequence of play and the cumulative loss of some fixed strategy. 


\begin{definition}
The regret of an algorithm running for $T$ iterations relative to some competing strategy $y \in \mathcal{S}$, is defined as
\[regret_T(y) = \sum_{t=1}^{T}f_t(x_t) - \sum_{t=1}^{T}f_t(y)\]
\end{definition}

Clearly, we want to minimize the regret with respect to the best fixed strategy in hindsight. So relative to the set of competing strategies $\mathcal{S}$ the regret of an online algorithm is defined as 

\begin{equation*}
    regret_T(\mathcal{S}) = \max_{y \in \mathcal{S}} regret_T(y)
\end{equation*}

The learner's goal is to have the lowest regret possible. In fact, we say an algorithm exhibits no-regret if the difference between the cumulative loss of the sequence of play and the cumulative loss of the best fixed strategy tends to zero as $T$, the number of iterations, goes to infinity.

\begin{definition}
    An algorithm exhibits no-regret iff $regret_T(\mathcal{S})$ grows sublinearly  with $T$, that is
    \[\text{no-regret} \Leftrightarrow regret_T(\mathcal{S}) = o(T)\]
\end{definition}
...
In the first round we can play any feasible strategy and ties can be broken arbitrarily. While this approach yields no-regret for some loss functions \cite[Cor. 2.2]{shalev}, FTL does not guarantee no-regret in all cases. Consider the following counterexample as in \cite[Ex. 2.2]{shalev}. \\
...
Apart from $t = 1$, where $z_t$ could actually be set arbitrarily in $[-1,1]$, the predictions of FTL will lead to $w_t = 1$ when $t$ is even and $w_t = -1$ when $t$ is odd. Therefore the cumulative loss of FTL will be $T$, while the cumulative loss of the fixed solution $u = 0 \in \mathcal{S}$ is $0$. Hence, the regret of FTL is $T$, which is obviously not sublinearly with $T$. \\
...
Intuitively, the OGD algorithm moves $x_t$ in the direction of the minimum of $f_t$, but not too much because it wants to remember the effect of $f_1,\dots f_{t-1}$. This way, OGD is a no-regret algorithm in as sense of definition \ref{def:noRegret} \cite[Cor. 2.7]{shalev}. 
...
As shown in \cite[Cor. 2.17]{shalev} the Euclidean projection has no impact on the regret. Since OGDLP is induced by Euclidean regularization it enjoys the same regret bound as in proposition \ref{prop:EuclideanNoRegret} where $B = 1$ as we consider the feasible set to be the probability simplex. Consequently, OGDLP is considered as a no-regret algorithm. The next algorithm yields the no-regret property while having a much simpler update rule. Instead of Euclidean it uses entropic regularization.
...
Just like OGDLP, NEG is a no-regret algorithm in the sense of definition \ref{def:noRegret} \cite[Cor. 2.14]{shalev}.
...
Note that in comparison with the regret bound of OGDLP as in proposition \ref{prop:EuclideanNoRegret} the regret bound of NEG also depends on the dimension of the predictions. As we constrained on the probability simplex we can set $B = 1$. \\

To sum up, we introduced two no-regret Algorithms that can be applied for simplex constrained convex optimization problems. First, Online Gradient Descent with Lazy Projections (OGDLP) which was based on Euclidean regularization and second, Normalized Exponentiated Gradient (NEG) induced by entropic regularization. We will run both algorithms in simple finite games later in chapter \ref{chapter:simulations}. Before that, lets define finite games and their equilibira concepts. 
